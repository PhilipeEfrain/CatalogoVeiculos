...
( function ( blocks, element, blockEditor ) {
    ...
    const block = registerBlockType( 'catalogo-veiculos/front-hero', {
        apiVersion: 2,
        title: 'Front Hero',
        icon: 'block-default',
        category: 'shop',
        ...
        edit: function ( props ) {
            const blockProps = useBlockProps({ className: 'background-cover bg-dark pb-5 position-relative pt-5 text-white', style: { backgroundImage: propOrDefault( props.attributes.background.url, 'background', 'url' ) ? ('url(' + propOrDefault( props.attributes.background.url, 'background', 'url' ) + ')') : null } });
        ...
        save: function(props) {
            const blockProps = useBlockProps.save({ className: 'background-cover bg-dark pb-5 position-relative pt-5 text-white', style: { backgroundImage: propOrDefault( props.attributes.background.url, 'background', 'url' ) ? ('url(' + propOrDefault( props.attributes.background.url, 'background', 'url' ) + ')') : null } });
            return el('section', { ...blockProps }, el('div', { className: 'container pb-5 pt-5' }, el('div', { className: 'pb-5 pt-5 row' }, el('div', { className: 'col-lg-7 pb-5 pt-5' }, [el(RichText.Content, { tagName: 'p', className: 'fw-normal h4 text-uppercase', value: props.attributes.sup_heading }), el(RichText.Content, { tagName: 'h1', className: 'display-3 fw-bold mb-3', value: propOrDefault( props.attributes.heading, 'heading' ) }), el(RichText.Content, { tagName: 'p', className: 'lead mb-4', value: props.attributes.tagline }), el(RichText.Content, { tagName: 'a', href: props.attributes.button_link.url, className: 'btn btn-light pb-2 ps-4 pe-4 pt-2', value: propOrDefault( props.attributes.button_label, 'button_label' ) })]))));
        }                        
