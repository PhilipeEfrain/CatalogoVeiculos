...
( function ( blocks, element, blockEditor ) {
    ...
    const block = registerBlockType( 'catalogo-veiculos/shop-front-cta', {
        apiVersion: 2,
        ...
        save: function(props) {
            const blockProps = useBlockProps.save({ className: ' pb-3 pt-3 text-center' });
            return el('section', { ...blockProps }, el('div', { className: 'container' }, el('div', { className: 'bg-dark text-white' }, el('div', { className: 'align-items-center row' }, [el('div', { className: 'col-lg-6 pb-5 pb-lg-0 pt-5 pt-lg-0' }, el('div', { className: 'ps-5 pe-5' }, [el(RichText.Content, { tagName: 'h2', className: 'fw-normal h5 mb-1 text-white-50', value: props.attributes.top_title }), el(RichText.Content, { tagName: 'h3', className: 'fw-bold h1 mb-4', value: propOrDefault( props.attributes.text, 'text' ) }), el(RichText.Content, { tagName: 'a', href: props.attributes.link.url, className: 'btn btn-light pb-2 ps-4 pe-4 pt-2', value: propOrDefault( props.attributes.label, 'label' ) })])), el('div', { className: 'col-lg-6' }, [props.attributes.image && props.attributes.image.svg && pgCreateSVG(RawHTML, {}, pgMergeInlineSVGAttributes(propOrDefault( props.attributes.image.svg, 'image', 'svg' ), { className: 'd-block img-fluid w-100', style: { maxHeight: '320px',objectFit: 'cover' } })), props.attributes.image && !props.attributes.image.svg && propOrDefault( props.attributes.image.url, 'image', 'url' ) && el('img', { src: propOrDefault( props.attributes.image.url, 'image', 'url' ), className: 'd-block img-fluid w-100 ' + (props.attributes.image.id ? ('wp-image-' + props.attributes.image.id) : ''), alt: propOrDefault( props.attributes.image?.alt, 'image', 'alt' ), width: '480', height: '360', style: { maxHeight: '320px',objectFit: 'cover' } })])]))));
        }                        
