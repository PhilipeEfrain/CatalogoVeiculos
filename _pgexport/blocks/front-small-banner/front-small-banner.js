...
( function ( blocks, element, blockEditor ) {
    ...
    const block = registerBlockType( 'catalogo-veiculos/front-small-banner', {
        apiVersion: 2,
        title: 'Front Small Banner',
        icon: 'block-default',
        category: 'shop',
        parent: [ 'catalogo-veiculos/front-small-banners' ],
        ...
        save: function(props) {
            const blockProps = useBlockProps.save({ className: 'col-md col-sm-6 py-3' });
            return el('div', { ...blockProps }, el('div', { className: 'position-relative text-dark' }, [props.attributes.image && props.attributes.image.svg && pgCreateSVG(RawHTML, {}, pgMergeInlineSVGAttributes(propOrDefault( props.attributes.image.svg, 'image', 'svg' ), { className: 'img-fluid w-100' })), props.attributes.image && !props.attributes.image.svg && propOrDefault( props.attributes.image.url, 'image', 'url' ) && el('img', { src: propOrDefault( props.attributes.image.url, 'image', 'url' ), className: 'img-fluid w-100 ' + (props.attributes.image.id ? ('wp-image-' + props.attributes.image.id) : ''), alt: propOrDefault( props.attributes.image?.alt, 'image', 'alt' ), width: '280', height: '200' }), el('div', { className: 'bottom-0 end-0  pb-5 pe-4 position-absolute ps-4 start-0' }, [el(RichText.Content, { tagName: 'h2', className: 'h5 mb-3', value: propOrDefault( props.attributes.title, 'title' ) }), el(RichText.Content, { tagName: 'a', href: propOrDefault( props.attributes.link_url.url, 'link_url', 'url' ), className: 'link-secondary small stretched-link text-decoration-none', value: propOrDefault( props.attributes.link_label, 'link_label' ) })])]));
        }                        
