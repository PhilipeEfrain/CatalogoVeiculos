...
<nav> 
    <ul class="justify-content-center  pagination"> 
        <li class="<?php if(!( PG_WCPagination::getCurrentPage() > 1 )) echo 'disabled'; ?> page-item"> <a class="link-secondary page-link" <?php echo PG_WCPagination::getPreviousHrefAttribute(); ?>><?php _e( 'Previous', 'catalogo_veiculos' ); ?></a> 
        </li>         
        <?php $dots = false; ?>
        <?php for( $page_num = 1; $page_num <= PG_WCPagination::getMaxPages(); $page_num++) : ?>
            ...
            <?php endif; ?>
        <?php endfor; ?>                   
        <li class="<?php if(!( PG_WCPagination::getCurrentPage() < PG_WCPagination::getMaxPages() )) echo 'disabled'; ?> page-item"> <a class="link-secondary page-link" <?php echo PG_WCPagination::getNextHrefAttribute(); ?>><?php _e( 'Next', 'catalogo_veiculos' ); ?></a> 
        </li>         
    </ul>     
</nav>